      ...
      protected Element videoQueue { get; set; }     
       ...
       // 
        ...
        protected void StartBusMessagePump()
        {
            ...
            {
                if (attached) { try { messagePump.Bus.SyncMessage -= Monitor; } catch { } }
        ...
        protected void StopBusMessagePump()
        {
            ...
            if (messagePump == null) return;
            try { messagePump.Bus.SyncMessage -= Monitor; } catch { }
            try { messagePump.Bus.DisableSyncMessageEmission(); } catch { }
            try { messagePump.Dispose(); }
            catch (Exception ex) { LoggerManager.LogException(ex, "Could not stop GStreamer bus message pump"); }
        }
